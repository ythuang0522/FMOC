...
// Class: FMExtendTree
FMExtendTree::FMExtendTree(const std::string& query,
...
int FMExtendTree::extendOneBase(int direction)
{
	if(direction==0)
	{
...
void FMExtendTree::extendLeaves(int direction)
{
	if(direction==0)
	{
...
void FMExtendTree::attempToExtend_L(FMEONodePtrList &newLeaves)
{
...
void FMExtendTree::attempToExtend_R(FMEONodePtrList &newLeaves)
{
...
std::vector<std::pair<std::string, BWTIntervalPair> > FMExtendTree::getLeftFMIndexExtensions(FMExOverlapNode* pNode)
{
...
std::vector<std::pair<std::string, BWTIntervalPair> > FMExtendTree::getRightFMIndexExtensions(FMExOverlapNode* pNode)
{
    std::vector<std::pair<std::string, BWTIntervalPair> > out;
...
/*
bool FMExtendTree::isEnoughOverlap(FMExOverlapNode* pNode)
{
...
*/
/*
void FMExtendTree::OutResult(FMExOverlapNode* pNode)
{
	int flag=1;
...
bool FMExtendTree::PrunedBySeedSupport(int directflag)
{
	FMEONodePtrList newLeaves;
	//int currSeedIdx;
    ...
    {	
		...
		if(!(*iter)->isOverthreshold)
		{
			if( !( (*iter)->tempMismathCount <=(int)(indelOffset+m_seedSize) ))
			{
				...
				int now_length = (int)(*iter)->getLeftExtend().length()+(int)m_Kmer.length()+(int)(*iter)->getRightExtend().length();
				...
				(*iter)->seed_rate= 1-((double)((*iter)->insertion_count+(*iter)->deletion_count + (*iter)->mismatch_count+(*iter)->tempMismathCount )/(double)now_length );
...
bool FMExtendTree::isSupportedByNewSeed_L(FMExOverlapNode* currNode,size_t smallSeedIdx, size_t largeSeedIdx)
{
...
// Identify new seeds wrt currSeedIdx
bool FMExtendTree::isSupportedByNewSeed_R(FMExOverlapNode* currNode,size_t smallSeedIdx, size_t largeSeedIdx)
{
bool isNewSeedFound = false;
...
bool FMExtendTree::isTerminated_L()
{
	bool found=false;
	FMEONodePtrList newLeaves;
...
bool FMExtendTree::isTerminated_R()
{
	bool found=false;
	FMEONodePtrList newLeaves;
...
void FMExtendTree::OutToCorrect(FMExOverlapNode* currNode)
{
	...
	//printf("EX_str=\t%s\t%.2f\tLength=%d\tNumofMatchSeed=\t%d\n",out_string.c_str(),((double)currNode->numOfMatchSeeds)/((double)out_string.length()),(int)out_string.length(),(int)currNode->numOfMatchSeeds);
	//printf("out_string+length=\t%d\n",(int)out_string.length());
	int start_point=(int)m_initSeedoffset-(int)currNode->getLeftExtend().length();
	//printf("I_end+Start_point=\t%d\tCorrect_array_size=\t%d\n",(int)(out_string.length()-m_seedSize)+start_point,(int)m_correct_ksub.size());
	
	//if( ((int)(out_string.length()-m_seedSize)+start_point) >= (int)m_correct_ksub.size())
	//	printf("OutToCorrect Error\n");
...
void FMExtendTree::Left2Right()
{
	LDtoRD();
}
void FMExtendTree::LDtoRD()
{
	m_leaves.clear();
	...
	for(FMEONodePtrList::iterator iter = m_leaves.begin(); iter != m_leaves.end(); ++iter)
    {
		...
		(*iter)->lastSeedIdx=m_initSeedoffset+(m_Kmer.length()-m_seedSize);
